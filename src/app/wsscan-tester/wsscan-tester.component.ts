...
export class WsscanTesterComponent implements OnDestroy {
    ...
    /**
     * Gets the currently selected Server Type.
     */
    public get serverType(): string {
        return this.scanConfigFG.get('serverType').value;
    }
    ...
    public get serverUrl(): string {
        const serverType = this.scanConfigFG.get('serverType').value;
        const url = this.scanConfigFG.get('serverUrl').value;
        ...
        const protocol = 'ws';
        const port = serverType == 'manager' ? 8001 : 60001;
        const path = serverType == 'manager' ? 'ManagerService' : 'ScanningService';
     ...
     * Constructor.
     */
    constructor(public socket: ServerSocketService, private images: ImagesService) {
        ...
        this.scanConfigFG = fb.group({
            ...
            serverType: ['manager', Validators.required],
    ...
    showScanDialog(): void {
        const command: IScannerCommand = {
            ...
            parameters: {
                ...
                resolution: this.scanConfigFG.get('ppi').value,
    ...
    scan(): void {
        const command: IScannerCommand = {
            name: 'scan',
            parameters: {
                device: this.scanConfigFG.get('selectedDevice').value,
                source: this.scanConfigFG.get('documentSource').value,
                format: this.scanConfigFG.get('format').value,
                dialogTitle: this.scanConfigFG.get('dialogTitle').value,
                resolution: this.scanConfigFG.get('ppi').value,
                paper: this.scanConfigFG.get('paperSize').value,
    ...
    connect(): void {
        this.disconnect();
        this.socket.wsScanUrl = this.serverUrl;
        this.socket.connect();
        this.connectionSubscription = this.socket.connection.connectionStatus.subscribe(connections => {
            const serverUrlControl = this.scanConfigFG.get('serverUrl');
            const serverTYpeControl = this.scanConfigFG.get('serverType');

            serverUrlControl.enable();
            serverTYpeControl.enable();

            if (connections > 0) {
                serverUrlControl.disable();
                serverTYpeControl.disable();
            }
        });
        this.socketSubscription = this.socket.connection.messages.share()
            .retryWhen(errors => errors.delay(5000)).share().subscribe((responseStr: string) => {
                ...
                switch (response.type) {
                    case 'message':
                        ...
                        break;
                    case 'image':
                        const images = this.images.imageList.getValue();
     ...
     */
    refreshDevices() {
        this.socket.send({name: 'list-devices'});
    }
     ...
     */
    disconnect(): void {
        this.scanConfigFG.get('serverUrl').enable();
        this.scanConfigFG.get('serverType').enable();
